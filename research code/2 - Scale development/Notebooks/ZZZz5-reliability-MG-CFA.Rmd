...
my_dir <- "/Users/anne-flore/git/readability-rebased/Supplementary-material-folder/2 - Scale development/" #eg: "absolute_path_to/Supplementary-material-folder/2 - Scale development/"
...
# Omega check on all data

```{r omega simple}
cat("Omega on aggregated data for 4 factors")
corrM <- make_corrM(data_all_factors, nb_fa = 4)
om <- omega(corrM, fm="pa", rotate = "Promax", nfactors=4)

# corrFiml <- corFiml(data)
# omegaFromSem(this_fit,m=corrFiml,flip=TRUE,plot=TRUE)
...
# fit_Indices <- fitMeasures(this_fit, c("rmsea","rmsea.pvalue","rmsea.ci.lower","rmsea.ci.upper","rmsea.ci.level","srmr","cfi","chisq","df","pvalue"))
# 
# knitr::kable(fit_Indices,
#              caption = "Reduced model fit indices")


```
#### RMSEA (Root Mean Square Error of Approximation): our main indice
...
# build a model for a latent trait and 4 factors

# Create a string for the overall readability factor
overall_readability <- paste(unlist(Readability_factors), collapse = " + ")

# Create strings for each individual factor
factor_strings <- lapply(names(Readability_factors), function(factor_name) {
  paste(factor_name, " =~ ", paste(unlist(Readability_factors[[factor_name]]), collapse = " + "))
})

# Combine overall and individual factor strings
global_model_string <- c("readability =~ ", overall_readability, factor_strings)
global_model_string <- paste(global_model_string, collapse = "\n")
print(global_model_string)

global_fit <- cfa(global_model_string, data = grp_data, missing = "fiml", group = "stimulus")

omegaFromSem(global_fit,m=corrFiml,flip=TRUE,plot=TRUE)
# library(sem) # for omega
# omegaFromSem(global_fit,plot=TRUE)
