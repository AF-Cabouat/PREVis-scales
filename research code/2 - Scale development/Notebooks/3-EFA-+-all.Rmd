...
participantResponseFiles <- list.files(path= "./data",pattern = "\\.csv$")
...
analyze_stimuli <-function(num){
  #load one file
  my_data <- read.csv(paste("data/",participantResponseFiles[[num]],sep=""), encoding="UTF-8")
...
#For debugging we can set the stimuliCount to whatever we want
#stimuliCount <- 1

# we will store all dfs
dfs <- NULL
...
```
```{r load one file, include=FALSE}

my_data <- read.csv(paste("data/",participantResponseFiles[[7]],sep=""), encoding="UTF-8")
data <- subset(my_data, select = -c(seed)) 
mi <- mifa(
    data      = data, 
    n_pc      = 1:29, #factors estimates
...
```{r run one EFA, include=FALSE}
efa5 <- fa(corrM, nfactors = 4, rotate = "promax", fm = "ml", oblique.scores = TRUE)


efa <- (unclass(efa5))

dimnames_list <- attr(efa$loadings, "dimnames") # Extract dimension names of loadings object
rows <- data.frame(Item = dimnames_list[[1]])
columns <- data.frame(ML_Factor = dimnames_list[[2]])
loadings_df <- data.frame(Item = rows$Item, stringsAsFactors = FALSE) # Create an empty data frame to store the data

# Loop through columns to fill the data frame
for (j in seq_along(columns$ML_Factor)) {
  loadings_df <- cbind(loadings_df, efa$loadings[, j])
}

# Rename the columns in the new data frame
colnames(loadings_df) <- c("Item", columns$ML_Factor)
...
# this_data <- subset(ratings.7, select = -c(seed))
# mi <- mifa(
#     data      = this_data, 
#     n_pc      = 1:29, #factors estimates
#     ci        = "fieller", #confidence interval method
...
# corr_MICE <- cov2cor(mi$cov_combined)
# 
# bart <- cortest.bartlett(corr_MICE, n = nrow(this_data))
# bart
